...
const AddProduct: React.FC<AddProductProps> = ({
  ...
  const [showEditChoisesInput, setShowEditChoisesInput] = useState({
    status: false,
    index: -1,
  });
  ...
  // Helper component for tier price inputs to avoid repetition
  const TierPriceInput = ({ field, label }: { field: any; label: string }) => {
  ...
  const FormSchema = z.object({
    ...
    product_SKU: z
      .string()
      .min(1, { message: "SKU Dibutuhkan." })
      .max(20, { message: "SKU must be less than 20 characters." }),
    product_price: z.string().min(1, { message: "Masukkan Harga Beli." }), // Harga beli (modal)
    price_non_member: z.string().optional(),
    price_member: z.string().optional(),
    price_member_usaha: z.string().optional(),
    product_weight: z.string().min(1, { message: "Masukkan Berat." }),
    product_etalase: z
      .array(z.string())
      .nonempty({ message: "Minimal satu etalase harus dipilih." }),
    product_description: z
      .string()
      .max(100, { message: "Deskripsi harus kurang dari 100 karakter." })
      .optional(),
  });
  ...
  async function onSubmit(data: z.infer<typeof FormSchema>) {
    ...
    formData.append(
      "product_weight",
      (data.product_weight + quantity).toString()
    );
    ...
    // Tambahkan harga per-role ke formData
    if (data.price_non_member)
      formData.append("price_non_member", data.price_non_member);
    if (data.price_member) formData.append("price_member", data.price_member);
    if (data.price_member_usaha)
      formData.append("price_member_usaha", data.price_member_usaha);
  ...
  const FormSchemaEtalase = z.object({
    showcase_name: z
      .string()
      .nonempty("Nama etalase wajib diisi")
  ...
  const addProductHandler = async () => {
    ...
    try {
      ...
      if (koperasiId && newProductId) {
        ...
        // Tambahkan harga per-role jika ada di data
        if (mergedData.price_non_member)
          koperasiPostBody.price_non_member = Number(
            mergedData.price_non_member
          );
        if (mergedData.price_member)
          koperasiPostBody.price_member = Number(mergedData.price_member);
        if (mergedData.price_member_usaha)
          koperasiPostBody.price_member_usaha = Number(
            mergedData.price_member_usaha
          );

        try {
          await axiosInstance.post(
            `/koperasi/${koperasiId}/catalog/items`,
            koperasiPostBody
          );
          console.log(
            `Successfully linked product ${newProductId} to koperasi ${koperasiId}`
          );
        } catch (koperasiError) {
          console.error(
            "Failed to link product to koperasi catalog:",
            koperasiError
          );
        }
      }
  ...
  return (
    <>
      <div
        className={`${showNotification ? "hidden" : "block"} pt-5 w-full mb-32`}
      >
        ...
        <div className="w-full mt-10">
          <p className="font-semibold">
            <span className="text-gray-500">
              {section.detailProduct ? "2" : "1"}/2:
            </span>{" "}
            {section.addProduct ? "Informasi Product" : "Detail Product"}
          </p>

          <div className="flex items-center gap-3 mt-2">
            <div
              className={`${
                section.addProduct ? "bg-orange-500" : "bg-gray-300"
              } w-full h-1.5 transition-all rounded-full`}
            ></div>

            <div
              className={`${
                section.detailProduct ? "bg-orange-500" : "bg-gray-300"
              } w-full h-1.5 transition-all rounded-full`}
            ></div>
          </div>
        </div>
        ...
        <Form {...form}>
          <form
            ...
            className={`${
              section.detailProduct ? "hidden" : "block"
            } space-y-10 mt-5 bg-white p-5 rounded-lg`}
          >
            {/* Photo */}
            <FormField
                    ...
                    <FormControl>
                      <Input
                        ...
                        onChange={(e) => {
                          ...
                          setDisplayValue(formatRupiah(limitedValue));
            ...
            {/* Tier Prices - Only for Induk Koperasi */}
            {/* {affiliation?.affiliation === "KOPERASI_INDUK" && (
              <div className="space-y-4 p-4 border rounded-lg">
                <h3 className="font-medium">Harga Jual (Berdasarkan Role)</h3>
                <FormField
                  control={form.control}
                  name="price_member"
                  render={({ field }) => (
                    <TierPriceInput field={field} label="Harga Jual Anggota" />
                  )}
                />
                <FormField
                  control={form.control}
                  name="price_member_usaha"
                  render={({ field }) => (
                    <TierPriceInput
                      field={field}
                      label="Harga Jual Anggota Usaha"
                    />
                  )}
                />
                <FormField
                  ...
                  render={({ field }) => (
                    <TierPriceInput field={field} label="Harga Jual Umum" />
                  )}
                />
              </div>
            )} */}
              ...
              render={() => (
                <FormItem data-aos="fade-up">
                  <FormLabel className="flex items-center gap-5">
                    ...
                    <button
                      onClick={() => setShowPopUpAddEtalase(true)}
                      className="p-2 text-white bg-orange-500 rounded-lg"
                      type="button"
                    >
                      + Tambah Etalase
                    </button>
                  ...
                  {etalases
                    ?.filter(
                      (etalase) => etalase?.showcase_name !== "Semua Produk"
                    )
                    .map((etalase, index) => (
                      <label
                        key={etalase.showcase_id || `etalase-${index}`}
                      ...
                      >
                        <input
                          ...
                          checked={selectedEtalase.includes(
                            etalase.showcase_id
                          )}
                          onChange={() =>
                            handleCheckboxChange(etalase.showcase_id)
                          }
                        />
        ...
        <div
          className={`${
            section.detailProduct && !showAddVariant ? "flex" : "hidden"
          } mt-5 bg-white p-5 rounded-lg w-full flex-col gap-5`}
        >
          <div>
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                onClick={() =>
                  setShowField({ ...showField, stock: !showField.stock })
                }
            ...
            <div
              className={`${
                showField.stock ? "flex" : "hidden"
              } flex-col mt-5 items-center gap-3`}
            >
              ...
              <p className="text-gray-500">
                Kamu akan diingatkan saat produk kurang dari stok minimum.
              </p>
            </div>
          </div>
          ...
          <div>
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                onClick={() =>
                  setShowField({ ...showField, variant: !showField.variant })
                }
            ...
            <div
              className={`${
                showField.variant ? "flex" : "hidden"
              } w-full flex-col mt-5 items-center gap-3`}
            >
              {selectedVariants.map((variant, i) => (
                <p
                  key={i}
                  className="flex items-center w-full gap-3 p-3 font-semibold border border-orange-500 rounded-lg"
                >
                  {
                    variants.find((v) => v.variant_id === variant.variant_id)
                      ?.variant_name
                  }
                </p>
              ))}
            ...
            >
              <Button
                ...
                className="w-full text-white bg-orange-500 border border-orange-500"
              >
                <p>Pilih Variant</p>
              </Button>
            </div>
          ...
          <Button
            onClick={() => {
              setSection({ addProduct: true, detailProduct: false });
            }}
          ...
          <Button onClick={addProductHandler} disabled={loading ? true : false}>
            Simpan
          </Button>
        </div>
          ...
          <div className="w-full gap-5 p-5 mt-5 bg-white rounded-lg">
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                aria-label="Tutup pilih variant"
              >
                <X />
              </button>
            </div>
            ...
            <Button
              onClick={() => setShowPopUpAddVariant(true)}
              className="w-full mt-5 text-orange-500 bg-orange-100"
            >
              + Tambah Variant Baru
            </Button>
            ...
            <div className="mt-5">
              {variants.map((variant, index) => (
                <label
                  ...
                  className="flex items-center gap-3 p-3 mt-5 font-semibold border border-orange-500 rounded-lg"
                >
                  <input
                    type="checkbox"
                    name="variant"
                    ...
                    checked={selectedVariants.some(
                      (selected) => selected.variant_id === variant.variant_id
                    )}
            ...
            <Button
              onClick={() => setShowAddVariant(false)}
              className="w-full mt-5 bg-green-400"
            >
              Submit
            </Button>
          </div>
      ...
      {showPopUpAddEtalase && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
          <div data-aos="fade-up" className="bg-white p-5 rounded-lg w-[90%]">
            <div className="flex items-center justify-between">
              <p className="text-xl font-semibold">Tambah Etalase</p>
              <button
                onClick={() => setShowPopUpAddEtalase(false)}
                aria-label="Tutup tambah etalase"
              >
                <X />
              </button>
            </div>
            ...
            <Form {...formEtalase}>
              <form
                onSubmit={formEtalase.handleSubmit(onSubmitEtalase)}
        ...
        <div className="fixed inset-0 z-50 flex items-start justify-center py-10 overflow-y-auto bg-black bg-opacity-50">
          <div data-aos="fade-up" className="bg-white p-5 rounded-lg w-[90%]">
            <div className="flex items-center justify-between">
              ...
              <button
                onClick={() => setShowPopUpAddVariant(false)}
                aria-label="Tutup tambah varian"
              >
                <X />
              </button>
            </div>
            ...
            <Form {...formVariant}>
              <form
                onSubmit={formVariant.handleSubmit(onSubmitVariant)}
                className="p-5 mt-6 space-y-8 bg-white rounded-lg"
              >
                {/* Name */}
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem data-aos="fade-up" data-aos-delay="100">
                      ...
                      <FormControl>
                        <Input placeholder="Masukkan nama varian" {...field} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
                ...
                <Button
                  ...
                  data-aos-delay="200"
                  type="button"
                  onClick={() => setShowChoisesInput(true)}
                  className="w-full text-orange-400 bg-transparent border-2 border-orange-400 hover:bg-transparent"
                >
                  Tambah Pilihan
                </Button>
                        ...
                        <div className="flex items-center justify-between gap-5 mt-3">
                          ...
                          <div
                            ...
                            className={`w-14 h-8 flex items-center bg-gray-300 rounded-full p-1 cursor-pointer ${
                              choise.show ? "bg-orange-400" : "bg-gray-300"
                            }`}
                          >
                            <div
                              className={`bg-white w-6 h-6 rounded-full shadow-md transform duration-300 ${
                                choise.show ? "translate-x-6" : "translate-x-0"
                              }`}
                            ></div>
                          </div>
                        </div>
                ...
                {showChoisesInput && (
                  <div className="fixed inset-0 z-20 h-screen -translate-y-8 bg-black bg-opacity-50">
                    <div
                      data-aos="fade-up"
                      className="absolute bottom-0 w-full p-4 mt-10 bg-white rounded-t-lg"
                    >
                      ...
                      <div className="mt-5">
                        ...
                        <div
                          ...
                          className={`w-14 h-8 mt-3 flex items-center bg-gray-300 rounded-full p-1 cursor-pointer ${
                            showChoice ? "bg-orange-400" : "bg-gray-300"
                          }`}
                        >
                          <div
                            className={`bg-white w-6 h-6 rounded-full shadow-md transform duration-300 ${
                              showChoice ? "translate-x-6" : "translate-x-0"
                            }`}
                          ></div>
                        </div>
                      </div>
                ...
                {showEditChoisesInput.status && (
                  <div className="fixed inset-0 z-20 h-screen -translate-y-8 bg-black bg-opacity-50">
                    <div
                      data-aos="fade-up"
                      className="absolute bottom-0 w-full p-4 mt-10 translate-y-10 bg-white rounded-t-lg"
                    >
                      <p className="mb-10 text-lg font-semibold text-center">
                        Ubah Pilihan
                      </p>
                      ...
                      <div>
                        ...
                        <Input
                          ...
                          placeholder="Nama Pilihan"
                          value={
                            displayChoises[showEditChoisesInput.index].name
                          }
                          onChange={(e) => {
                            const updatedChoises = displayChoises.map(
                              (choise, index) =>
                                index === showEditChoisesInput.index
                                  ? { ...choise, name: e.target.value }
                                  : choise
                            );
                      ...
                      <div className="mt-5">
                        ...
                        <Input
                          ...
                          placeholder="Harga"
                          value={
                            displayChoises[showEditChoisesInput.index].price
                          }
                          onChange={(e) => {
                            const updatedChoises = displayChoises.map(
                              (choise, index) =>
                                index === showEditChoisesInput.index
                                  ? { ...choise, price: Number(e.target.value) }
                                  : choise
                            );
                      ...
                      <div className="mt-5">
                        ...
                        <div
                          ...
                          className={`w-14 h-8 mt-3 flex items-center bg-gray-300 rounded-full p-1 cursor-pointer ${
                            displayChoises[showEditChoisesInput.index].show
                              ? "bg-orange-400"
                              : "bg-gray-300"
                          }`}
                        >
                    ...
                    <FormItem data-aos="fade-up" data-aos-delay="300">
                      <div className="flex items-center justify-between gap-5">
                        ...
                        <FormControl>
                          <div
                            className={`w-14 h-8 flex items-center bg-gray-300 rounded-full p-1 cursor-pointer ${
                              field.value ? "bg-orange-400" : "bg-gray-300"
                            }`}
      ...
      {showNotificationEtalase && (
        <Notification
          message="Etalase berhasil ditambahkan!"
          onClose={() => setShowNotificationEtalase(false)}
          status="success"
        />
      )}

      {/* Success Notification for Variant */}
      {showNotificationVariant && (
        <Notification
          message="Varian berhasil ditambahkan!"
          onClose={() => setShowNotificationVariant(false)}
          status="success"
        />
      )}
      ...
      {showNotificationAddProduct && (
        <Notification
          ...
          onClose={() => setShowNotificationAddProduct(false)}
          status="success"
        />
      )}

      {/* Error Notification */}
      {showErrorForAddProduct && (
        <Notification
          message={message}
          onClose={() => setShowErrorForAddProduct(false)}
          status="error"
        />
      )}
      ...
      {showNotification && (
        <div className="p-10">
          ...
          <p
            data-aos="fade-up"
            data-aos-delay="100"
            className="mt-10 text-xl font-semibold text-center"
          >
          ...
          <Button
            ...
            className="w-full mt-10 text-white bg-green-500"
          >
            Done
          </Button>
        </div>
