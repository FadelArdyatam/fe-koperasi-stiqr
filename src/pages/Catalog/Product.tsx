...
const Product: React.FC<ProductProps> = ({
  koperasiId, // Destructure koperasiId
  products,
  setProducts,
  addProduct,
  setAddProduct,
  ...
  return (
    <div className="mb-32 px-5">
      <div className={`${addProduct || open.status ? "hidden" : "block"}`}>
          ...
          {products?.map((product, index) => (
            <div
              ...
              className={`relative ${
                ...
                product?.detail_product?.stok === 0
                  ? "bg-red-100"
                  : "bg-white"
              } flex w-full justify-between items-center p-4 gap-4 rounded-md shadow-sm mt-5 cursor-pointer hover:bg-orange-100 transition-all`}
              onClick={() => handleOpen(product.product_id)}
            >
              {product?.detail_product?.is_stok &&
                product?.detail_product?.stok <=
                  product?.detail_product?.stok_minimum &&
                product?.detail_product?.stok != 0 && (
        ...
        ${product.product_status ? "bg-orange-500" : "bg-gray-300"} 
        transition-colors`}
                onClick={(event) => {
                  event.stopPropagation();
          ...
          {products?.length === 0 && (
            <div className="flex justify-center gap-3 flex-col">
              <img
                className="md:w-3/12 w-2/3 place-items-center self-center mt-10"
                src={imgNoCatalog}
                alt="Katalog Kosong"
              />
              <p className="text-center text-orange-400 font-bold md:text-xl">
      ...
      {addProduct && (
        <AddProduct
          koperasiId={koperasiId} // Pass koperasiId down
          products={products}
