...
interface Product {
  ...
  tier: string;
  stock: number;
  merchant: { id: string; name: string; email: string; affiliation: string; approval_status: string };
}
...
interface MarginRule {
  tier: "NON_MEMBER" | "MEMBER" | "MEMBER_USAHA" | "UMUM";
  type: "FLAT" | "PERCENT";
  value: number;
}
...
const DialogCustomerSearch = ({
  ...
  const selectAndClose = (member: Member) => {
          ...
          <Card className="w-full max-w-md">
            ...
            <CardContent className="space-y-3">
              ...
              <div className="space-y-2 max-h-60 overflow-y-auto">
                <div
                  onClick={() =>
                    selectAndClose({ id: "non-member", name: "Non Anggota", email: "", phone_number: "", tier: "NON_MEMBER" })
                  }
                  className={`p-3 border rounded-lg cursor-pointer ${currentMember?.tier === "NON_MEMBER" ? "bg-orange-50 border-orange-400" : "hover:bg-gray-50"
                    }`}
                >
                  Non Anggota
                </div>
                {members.map((member) => (
                  <div
                    key={member.id}
                    onClick={() => selectAndClose(member)}
                    className={`p-3 border rounded-lg cursor-pointer ${currentMember?.id === member.id ? "bg-orange-50 border-orange-400" : "hover:bg-gray-50"
                      }`}
                  >
...
const KasirKoperasi: React.FC = () => {
  ...
  // Processing modal (used while waiting server respond/create QR)
  ...
  // default member on mount
  useEffect(() => {
    setSelectedMember({ id: "non-member", name: "Non Anggota", email: "", phone_number: "", tier: "NON_MEMBER" });
  }, []);
  ...
  const fetchProducts = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      const mapped: Product[] = items.map((it: any) => ({
        ...
        price_member_usaha:
          it.price_member_usaha !== undefined ? it.price_member_usaha : it.product?.price_member_usaha ?? null,
        product_image: it.product?.product_image ?? it.product_image ?? it.image ?? undefined,
        stock: Number(it.product?.stock ?? it.stock ?? 99999),
        merchant: it.product?.merchant ?? {},
        tier: "NON_MEMBER",
        product_description: it.product?.product_description ?? "",
      }));
      setProducts(mapped);
    } catch (error) {
      console.error("Error fetching products:", error);
      setNotification({ message: "Gagal memuat produk.", status: "error" });
    } finally {
      setLoading(false);
    }
  }, [koperasiId]);
  ...
  const fetchMembers = useCallback(async (q: string): Promise<void> => {
    ...
    try {
      ...
      const membersData: Member[] = membersArray.map((m: any) => ({
        id: m.id,
        name: m.name || m.user?.username || m.email || "Unknown",
        email: m.email || "",
  ...
  // Combined initial load for cleaner UX
  useEffect(() => {
    if (koperasiId && !affiliationLoading) {
      setInitialLoading(true);
      Promise.all([fetchProducts(), fetchMembers(""), fetchMarginRules()])
        .catch((e) => console.error(e))
        .finally(() => setInitialLoading(false));
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [koperasiId, affiliationLoading]);
  ...
  useEffect(() => {
    ...
    const handlePaymentSuccess = (data: any) => {
      if (data.orderId === currentOrderId) {
        ...
        setShowReceipt(true);
        setLastOrderData((prev: any) => ({ ...(prev || {}), payment_method: data.payment_method, order_id: data.orderId }));
        setNotification({
  ...
  const computeFinalPriceForProduct = useCallback(
    (product: Product, memberTier?: string): number => {
  ...
  const { total, margin } = useMemo(
    () =>
      basket.reduce(
        (acc, item) => {
          acc.total += item.finalPrice * item.quantity;
          acc.margin += item.margin_amount * item.quantity;
          return acc;
        },
        { total: 0, margin: 0 }
      ),
  ...
  const updateBasketQuantity = (productId: string, newQuantity: number) => {
    setBasket((prev) => {
      ...
      if (existing) {
        return prev.map((item) => (item.product_id === productId ? { ...item, quantity: newQuantity } : item));
      } else {
  ...
  // Handle payment method select with clearer processing flow
  const handlePaymentMethodSelect = async (method: "CASH" | "QRIS") => {
    ...
    // If no items
    if (basket.length === 0 || basket.every((b) => b.quantity <= 0)) {
      setNotification({ message: "Keranjang kosong.", status: "warning" });
      return;
    }

    // Prepare UI states for processing
    ...
    try {
      ...
      if (response.data.success && response.data.data) {
        // Set last order (include payment method)
        const respData = response.data.data;
        const combined = { ...checkoutData, ...respData, payment_method: method };
        setLastOrderData(combined);
        setCurrentOrderId(respData.order_id);

        if (method === "QRIS") {
          // For QRIS, if server returns QR immediately show it; otherwise wait for websocket
          setPaymentStatus("PENDING");
          setTimeLeft(300);
          if (respData.qr_code) {
            setQrCode(respData.qr_code);
            setShowQRModal(true);
            setShowProcessingModal(false);
          } else {
            // Show waiting modal until websocket emits qris:generated
            setShowQRISWaiting(true);
            setShowProcessingModal(false);
            setQrCode(null);
          }
        } else {
          // CASH: finalize immediate receipt view
          setShowProcessingModal(false);
          setShowReceipt(true); // Directly show receipt
        //   setNotification({
      ...
      if (error.response?.data?.message) {
        ...
        if (message.toLowerCase().includes("qris") || message.toLowerCase().includes("pengajuan")) {
  ...
  const resetTransaction = (status?: "cancelled" | "completed") => {
    ...
    // Optionally clear last order data after closing receipt
    setLastOrderData(null);
  };

  // If affiliation still loading show small message
  ...
  if (initialLoading) {
    return (
      <div className="flex flex-col items-center justify-center h-screen bg-white">
        <div className="animate-spin h-12 w-12 border-b-4 border-orange-500 rounded-full mb-4"></div>
  ...
  return (
    <>
      {/* Inline small animation styles */}
      <style>{`
        @keyframes fadeIn {
          from { opacity: 0; transform: translateY(4px) scale(.99); }
          to { opacity: 1; transform: translateY(0) scale(1); }
        }
        .animate-fadeIn { animation: fadeIn .22s ease-in-out; }
      `}</style>

      {notification && <Notification message={notification.message} status={notification.status} onClose={() => setNotification(null)} />}

      {/* Main Layout - hides when modals open */}
      <div className={`${showPaymentModal || showQRModal || showReceipt || showProcessingModal || showQRISWaiting ? "hidden" : "flex"} w-full pb-40 flex-col min-h-screen items-center bg-orange-50`}>
        <div className="p-5 w-full">
          ...
          <div className="w-full flex items-center gap-5 justify-between">
            ...
            <Button onClick={() => setShowPaymentModal(true)} className="bg-orange-100 rounded-full text-orange-500 p-2">
              + Input Manual
            </Button>
          </div>
          ...
          <div className="mt-5 p-3 border rounded-lg bg-white">
            <div className="flex items-center gap-3">
              <div
                className={`p-2 rounded-full ${selectedMember?.tier === "MEMBER" ? "bg-blue-100 text-blue-600" : selectedMember?.tier === "MEMBER_USAHA" ? "bg-orange-100 text-orange-600" : "bg-gray-200 text-gray-600"
                  }`}
              >
                ...
                <div key={product.id} className="bg-white rounded-lg p-4 shadow-sm">
                  <div className="flex items-center gap-5 flex-1 cursor-auto overflow-hidden">
                    ...
                    <div className="h-12 w-12 min-w-12 bg-gray-200 rounded-md overflow-hidden">
                      <img src={product.product_image || noProduct} alt={product.product_name} className="h-full w-full object-cover rounded-md" />
                    </div>

                    {/* Product Info */}
                    <div className="flex flex-col justify-start items-start min-w-0 overflow-hidden flex-grow">
                      <p className="text-lg font-semibold text-start truncate w-full">{product.product_name}</p>
                      <p className="font-semibold text-orange-600">{formatRupiah(computeFinalPriceForProduct(product, selectedMember?.tier))}</p>
                      {product.stock !== undefined && <span className="bg-orange-100 px-3 py-1 mt-1 rounded-full text-orange-500 font-normal text-xs">Stok: {product.stock}</span>}
                    </div>
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        onClick={() =>
                          updateBasketQuantity(product.product_id, Math.max(0, (basket.find((item) => item.product_id === product.product_id)?.quantity || 0) - 1))
                        }
                      ...
                      <button
                        onClick={() =>
                          updateBasketQuantity(product.product_id, (basket.find((item) => item.product_id === product.product_id)?.quantity || 0) + 1)
                        }
                        className="w-8 h-8 flex items-center justify-center text-2xl font-semibold rounded-full bg-orange-100"
                      >
                        +
        ...
        {basket.length > 0 && basket.some((item) => item.quantity > 0) && (
          <Button
            ...
            className="fixed w-[90%] bottom-5 rounded-full left-[50%] -translate-x-[50%] bg-green-500 text-white px-5 py-[25px] flex items-center justify-between shadow-lg"
          >
      ...
      {showPaymentModal && (
        <div className="fixed w-full h-full bg-black bg-opacity-50 top-0 left-0 z-20 flex items-end justify-center">
          <div className="w-full bg-white rounded-t-xl p-5 animate-fadeIn">
            ...
            <div className="mb-5">
              ...
              <Input
                ...
                pattern="[0-9]*"
              />
              <p className="text-xs text-gray-500 mt-1">{pin.length}/6 digit</p>
            </div>
            ...
            <div className="space-y-3">
              ...
              >
                {paymentLoading && paymentMethod === "CASH" ? <div className="animate-spin rounded-full h-5 w-5 border-b-2 border-orange-500 mr-3"></div> : <Wallet className="mr-3" size={20} />}
                <div className="text-left">
                  <div className="font-semibold">{paymentLoading && paymentMethod === "CASH" ? "Memproses..." : "Bayar Tunai"}</div>
              ...
              >
                {paymentLoading && paymentMethod === "QRIS" ? <div className="animate-spin rounded-full h-5 w-5 border-b-2 border-blue-500 mr-3"></div> : <QrCode className="mr-3" size={20} />}
                <div className="text-left">
                  <div className="font-semibold">{paymentLoading && paymentMethod === "QRIS" ? "Memproses..." : "QRIS"}</div>
      ...
      {/* Processing Modal (shown while waiting server) */}
      {showProcessingModal && (
        <ModalTemplate title="Menyiapkan Pembayaran..." onClose={() => resetTransaction("cancelled")}>
          <CardContent className="text-center">
            ...
            {currentOrderId && <div className="text-xs text-gray-500">Order ID: {currentOrderId}</div>}
          </CardContent>
          <CardFooter>
            <Button variant="destructive" onClick={() => resetTransaction("cancelled")}>Batal</Button>
          </CardFooter>
        </ModalTemplate>
      )}

      {/* QRIS Waiting Modal */}
      {showQRISWaiting && !qrisError && (
        <ModalTemplate title="Menunggu QRIS" onClose={() => resetTransaction("cancelled")}>
          <CardContent className="text-center">
            <div className="animate-spin rounded-full h-16 w-16 border-b-2 border-orange-500 mx-auto mb-4"></div>
          ...
          <CardFooter className="flex gap-2">
            ...
            <Button onClick={() => { setShowQRISWaiting(false); setQrisError(null); setShowPaymentModal(true); }}>Ganti Metode</Button>
          </CardFooter>
        </ModalTemplate>
      )}
      ...
      {/* QRIS Error Modal */}
      {showQRISWaiting && qrisError && (
        <ModalTemplate title="QRIS Tidak Tersedia" onClose={() => resetTransaction("cancelled")}>
          <CardContent className="text-center">
      ...
      {/* QR Modal (show QR image + countdown + status) */}
      {showQRModal && qrCode && (
        <ModalTemplate title="Scan untuk Membayar" onClose={() => resetTransaction("cancelled")}>
          <CardContent className="text-center">
            <div className="mb-4">
      ...
      {/* Receipt Modal (unified struk for CASH & QRIS) */}
      {showReceipt && lastOrderData && (
        <ModalTemplate title="Transaksi Berhasil" onClose={() => resetTransaction("completed")}>
          <CardContent className="pt-4 text-sm space-y-3">
            <div className="text-center">
              <CheckCircle className="w-12 h-12 mx-auto text-green-500 mb-2" />
              <h3 className="font-bold text-lg">Struk Pembayaran</h3>
              <p className="text-gray-500 text-xs">
                Metode: {String(lastOrderData.payment_method || paymentMethod || "TUNAI").toUpperCase()}
              </p>
            </div>
            <div className="border-t pt-3">
              <p><strong>Order ID:</strong> {lastOrderData.order_id ?? currentOrderId}</p>
              <p><strong>Pelanggan:</strong> {lastOrderData.customer_info?.name ?? selectedMember?.name}</p>
              <p><strong>Tanggal:</strong> {new Date().toLocaleString("id-ID")}</p>
            </div>
            <div className="border-t pt-2">
              {lastOrderData.items?.map((it: any, i: number) => (
                <div key={i} className="flex justify-between">
                  <span>{products.find(p => p.product_id === it.product_id)?.product_name || it.product_id} x{it.quantity}</span>
            ...
            <div className="border-t pt-2 font-bold flex justify-between text-base">
              <span>Total</span>
              <span className="text-orange-600">{formatRupiah(lastOrderData.order_summary?.total ?? total)}</span>
            </div>
          </CardContent>
          <CardFooter>
            <Button onClick={() => resetTransaction("completed")} className="w-full bg-green-500 hover:bg-green-600">
